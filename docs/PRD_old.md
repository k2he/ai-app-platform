...
│                          ROUTER TYPES                                        │
│  ┌────────────┐ ┌────────────┐ ┌────────────┐ ┌────────────────────────────┐│
│  │ llm_intent │ │   direct   │ │custom_result│ │     (team-defined)       ││
│  └────────────┘ └────────────┘ └────────────┘ └────────────────────────────┘│
└─────────────────────────────────────────────────────────────────────────────┘
                                      │
         ┌────────────────────────────┼────────────────────────────┐
...
workflow:
  ...
  model: string                   # Default LLM model (gpt-4o, claude-3-opus, etc.)
...
|------|-------------|--------------|
| `llm_intent` | LLM classifies user intent | Platform sends conversation to LLM with intent options, returns matched route |
| `direct` | Unconditional routing | Always routes to specified node |
| `validation_result` | Route based on handler return | Handler returns `{result: "valid"}`, router matches to route key |
| `custom` | Team-defined routing logic | Handler returns route key directly |
